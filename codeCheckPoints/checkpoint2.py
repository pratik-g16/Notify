...
def home():
    ...
    if(os.path.exists(os.path.join('static/output/', 'output.zip'))):
        os.remove(os.path.join('static/output/', 'output.zip'))
...
def upload_image2(): 
    ...
    if file and allowed_file(file.filename): 
        ...
        if(option=='hw'):
            ...
            makeZip(["notes.pdf","notes.mp3","transcripts.pdf","notes.pptx"])
            # print("done1")
            # sendMail()
        else:
        ...
        # return "done"
        # sendMail()
        return flask.send_file("static\\output\\output.zip", as_attachment=True)
